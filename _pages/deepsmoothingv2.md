...
$$
\begin{align*}
    \mathcal{L}_0(\theta)&  \label{eq:pred_error}\tag{Prediction Error}\\ 
    &= \sqrt{\frac{1}{\vert \mathcal{I}_0\vert} \sum_{(\sigma,k,\tau)\in \mathcal{I}_0} \left(\sigma-\sigma_\theta(k,\tau)\right)^2}+\frac{1}{\vert \mathcal{I}_0\vert} \sum_{(\sigma,k,\tau)\in \mathcal{I}_0} \frac{\left\vert\sigma-\sigma_\theta(k,\tau)\right\vert}{\sigma} 
\end{align*}
$$
...
$$
\mathcal{T}_0 = \{\tau:(0,\tau)\in  \mathcal{I}_0\} \tag{$\mathcal{T}_0$}
$$

The other three loss functions are auxiliary, and consequently, we need to introduce auxiliary maturities and log moneyness values. This is to ensure desired features for the constructed implied variance $$\omega$$ across unobserved market data. Define
...
$$
\begin{align}
\mathcal{L}_{cal}(\theta) &= \frac{1}{\vert \mathcal{I}_{Aux}\vert}\sum_{(k,t)\in \mathcal{I}_{Aux}} \max\left(0,-\ell_{cal}(k,\tau)\right) \tag{Calendar Loss}\\
\mathcal{L}_{but}(\theta) &= \frac{1}{\vert \mathcal{I}_{Aux}\vert}\sum_{(k,t)\in \mathcal{I}_{Aux}} \max\left(0,-\ell_{but}(k,\tau)\right) \tag{Butterfly Loss}
\end{align}
$$
...
$$
\mathcal{L}_{asym}(\theta) = \frac{1}{|\mathcal{I}_{asym}|} \sum_{(k, \tau) \in \mathcal{I}_{asym}} \left\vert\frac{\partial^2 \omega_(k, \tau)}{\partial k \partial k}\right\vert
$$
...
We prefer that the prior component in \eqref{IV} gives the best possible fit for ATM, and the neural network fixes prior's limitations for OTM. To this end, the following loss function is considered.
...
$$
\mathcal{L}_{Tot}(\theta) = \mathcal{L}_0(\theta) + \lambda_{but}\mathcal{L}_{but} + \lambda_{cal}\mathcal{L}_{cal} + \lambda_{atm}\mathcal{L}_{atm} \tag{Total Loss}
$$
