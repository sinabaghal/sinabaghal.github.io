...
Numerical results show that the enhanced model, incorporating a neural network with the loss function $$\omega(k,\tau; \theta)$$, fits the Bates model data perfectly and produces an arbitrage-free volatility surface. The figure below shows how well the model fits the training data.
...
Click [here](https://github.com/sinabaghal/deepsmoothigIVS/blob/main/Technical_Report.pdf) to access a technical report with additional details. The project code is also available [here](https://github.com/sinabaghal/deepsmoothigIVS). For convenience, I include the main body of the code below as well. Please ensure you have utils.py from above link available to run it successfully.
