...
The figure below illustrates 29 trained implied volatility surfaces (IVS) obtained using the deep smoothing algorithm. Different values for the Bates model parameters are used in each case. Displayed parameters are $$\alpha, \beta, \kappa, v0, \theta, \rho, \sigma, \lambda$$ respectively. For definition of these parameters see the technical report inside the github repository. 
