...
to Italian games such as Cassino and Scopa, and more closely to the Egyptian game Bastra [**Wikipedia**](https://en.wikipedia.org/wiki/Pasur_(card_game)). This paper introduces a CUDA-accelerated computational framework for simulating Pasur, emphasizing efficient memory management. We use our framework to compute near-Nash equilibria via Counterfactual Regret Minimization (CFR), a well-known algorithm for solving large imperfect-information games.
